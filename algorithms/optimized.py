...
def obtain_possible_investment(
    ...
    """
    From a list of names, costs and profits of shares and a maximum amount of money (wallet)
    not to be exceeded. The function recursively calculates the best investment by storing
    the best profit obtained (best_profit) with the name of the shares purchased (best_investment).
    ...
    if node >= len(names):
        ...
        if best_profit[0] is None or profit_obtained >= best_profit[0]:
            ...
            best_investment[0] = (shares_purchased.copy(), round(500 - wallet, 2))
...
def optimized(
    ...
    """
    calls the function to recursively calculate the best investment and generates
    a printable message containing the profit obtained and the name of the shares to buy
    ...
    shares = "\n".join(best_investment[0][0])
    profit = round(best_profit[0], 2)
    expenses = round(best_investment[0][1], 2)
